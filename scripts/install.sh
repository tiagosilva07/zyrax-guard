#!/bin/sh
# Zyrax Guard installer:  curl -fsSL https://raw.githubusercontent.com/tiagosilva07/zyrax-guard/main/scripts/install.sh | sh
# Env: VERSION (default latest, or first arg), BINDIR, REPO.
set -eu
...
if have cosign && curl -fsSL -o "${tmp}/${asset}.cosign.bundle" "${base}/${asset}.cosign.bundle" 2>/dev/null; then
